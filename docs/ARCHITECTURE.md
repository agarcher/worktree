...
A planned feature to provide a stable, reusable numeric identifier for each worktree. This would be useful for:
- Port offsets (run multiple worktrees with different port ranges)
- Resource allocation

**Planned storage location:** `.git/worktrees/<name>/wt-index`

This leverages git's existing worktree metadata directory:
- Doesn't pollute the worktree with untracked files
